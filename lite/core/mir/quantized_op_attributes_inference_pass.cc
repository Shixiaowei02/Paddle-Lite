  ...
  for (auto& op_node : graph->StmtTopologicalOrder()) {
    ...
    for (auto out_var_node : op_node->outlinks) {
      ...
      for (auto out_op_node : out_var_node->outlinks) {
        ...
        auto input_scale =
            out_op_info->GetInputScale<OpAttrType::FLOAT>(out_var_node_name);
        if (!found) {
          found = true;
      ...
      if (found) {
        ...
        int bit_length = op_info->GetAttr<OpAttrType::INT>("bit_length");
        int range = (1 << (bit_length - 1)) - 1;
        output_scale = op_info->GetAttr<OpAttrType::FLOAT>("out_threshold");
